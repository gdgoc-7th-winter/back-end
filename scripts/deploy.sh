...
# ---- 설정 변수 ----
DEPLOY_DIR="${DEPLOY_DIR:-/opt/REDACTED_DB_PASSWORD/backend}"
ECR_REGISTRY="${ECR_REGISTRY:-873135413383.dkr.ecr.ap-northeast-2.amazonaws.com}"
REPO_NAME="${REPO_NAME:-REDACTED_DB_PASSWORD-backend}"
...
detect_active_slot() {
  ...
  if docker ps -q -f "name=REDACTED_DB_PASSWORD-backend-blue" --format '{{.Names}}' 2>/dev/null | grep -q .; then
    echo "blue"
    return
  fi
  if docker ps -q -f "name=REDACTED_DB_PASSWORD-backend-green" --format '{{.Names}}' 2>/dev/null | grep -q .; then
    echo "green"
    return
  fi
  # 레거시 마이그레이션: 기존 REDACTED_DB_PASSWORD-backend → blue로 간주
  if docker ps -q -f "name=REDACTED_DB_PASSWORD-backend" --format '{{.Names}}' 2>/dev/null | grep -q .; then
    echo "blue"
    return
  fi
  echo ""
...
if [[ "$HEALTH_OK" != "true" ]]; then
  ...
  docker rm -f "REDACTED_DB_PASSWORD-backend-$TARGET" 2>/dev/null || true
...
if docker ps -q -f "name=REDACTED_DB_PASSWORD-nginx" --format '{{.Names}}' 2>/dev/null | grep -q .; then
  docker exec REDACTED_DB_PASSWORD-nginx nginx -s reload 2>/dev/null || docker compose restart "$NGINX_SERVICE"
else
  echo "  Starting nginx..."
...
if [[ "$FIRST_DEPLOY" != "true" ]]; then
  ...
  # 레거시 마이그레이션: 기존 REDACTED_DB_PASSWORD-backend 컨테이너 정리
  if docker ps -a -q -f "name=REDACTED_DB_PASSWORD-backend" --format '{{.Names}}' 2>/dev/null | grep -q .; then
    docker stop REDACTED_DB_PASSWORD-backend 2>/dev/null || true
    docker rm REDACTED_DB_PASSWORD-backend 2>/dev/null || true
    echo "  Stopped legacy REDACTED_DB_PASSWORD-backend (migration)"
  fi
else
