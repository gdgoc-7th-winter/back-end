...
public interface UserAuthControllerDocs {
    ...
    CommonResponse<Void> sendEmail(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "이메일 발송 요청") EmailSendRequest request);

    @Operation(summary = "인증 코드 검증", description = "이메일과 인증 코드를 검증합니다.")
    @ApiResponse(responseCode = "200", description = "성공")
    CommonResponse<Void> verifyCode(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "인증 코드 검증 요청") EmailVerificationRequest request);
}
