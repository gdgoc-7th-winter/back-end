...
public class PromotionPostQueryServiceImpl implements PromotionPostQueryService {
    ...
    public Page<PromotionPostListResponse> getList(
        ...
        var tagNamesByPostId = postTagQueryService.getTagNamesByPostIds(
                page.getContent().stream().map(PromotionPostListQueryResult::postId).toList());

        return page.map(result -> new PromotionPostListResponse(
                result.category(),
                toPostListResponse(result, tagNamesByPostId.getOrDefault(result.postId(), List.of()))
        ));
    }

    private PostListResponse toPostListResponse(PromotionPostListQueryResult result, List<String> tagNames) {
