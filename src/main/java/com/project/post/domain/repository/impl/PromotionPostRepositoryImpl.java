...
import com.project.post.domain.entity.QPromotionPost;
import com.project.post.domain.enums.PromotionCategory;
import com.project.post.domain.repository.PromotionPostRepositoryCustom;
...
public class PromotionPostRepositoryImpl implements PromotionPostRepositoryCustom {

    private final JPAQueryFactory queryFactory;

    @Override
    public Page<PromotionPostListQueryResult> findPromotionPostList(
            @Nullable PromotionCategory category,
            @NonNull Pageable pageable) {
        QPromotionPost promotionPost = QPromotionPost.promotionPost;
        ...
        where.and(promotionPost.deletedAt.isNull());
        where.and(post.deletedAt.isNull());
        if (category != null) {
            where.and(promotionPost.category.eq(category));
        }

        var projection = Projections.constructor(
                PromotionPostListQueryResult.class,
        ...
        List<PromotionPostListQueryResult> content = queryFactory
                .select(projection)
                .from(promotionPost)
